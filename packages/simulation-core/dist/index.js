...
const parseArgs = () => {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        switch (arg) {
            ...
            case '-n':
                if (nextArgIsNumber)
                    parsed.numChargers = parseInt(nextArg, 10);
                i++;
                break;
            case '--powerKW':
            case '-p':
                if (nextArgIsNumber)
                    parsed.chargerPowerKW = parseFloat(nextArg);
                i++;
                break;
            case '--arrivalMultiplier':
            case '-m':
                if (nextArgIsNumber)
                    parsed.arrivalProbabilityMultiplier = parseFloat(nextArg);
                i++;
                break;
            case '--consumption':
            case '-c':
                if (nextArgIsNumber)
                    parsed.evConsumptionKWhPer100km = parseFloat(nextArg);
                i++;
                break;
            case '--seed':
            case '-s':
                if (nextArgIsNumber)
                    parsed.rngSeed = parseInt(nextArg, 10);
                i++;
                break;
            case '--durationDays':
            case '-d':
                if (nextArgIsNumber)
                    parsed.durationTicks = parseInt(nextArg, 10) * 24 * 4;
                i++;
                break;
            case '--concurrencyCurve':
                parsed.concurrencyCurve = true;
                break;
            case '--help':
            case '-h':
                parsed.help = true;
...
const runSingleSimulation = (config) => {
...
Object.defineProperty(exports, "runSimulation", { enumerable: true, get: function () { return simulation_2.runSimulation; } });
